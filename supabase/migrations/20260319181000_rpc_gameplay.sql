CREATE OR REPLACE FUNCTION public.activate_turbo_rpc()
...
BEGIN
  ...
  IF v_daily.turbo_last_used_day = v_today THEN
    v_activated_at := COALESCE(v_daily.turbo_activated_at, now());
    RETURN QUERY SELECT false, v_activated_at + interval '2 hours';
    RETURN;
  END IF;
  ...
  UPDATE daily_states
  SET
    heat_before_turbo = v_daily.heat,
    heat = 100,
    turbo_activated_at = v_activated_at,
    turbo_last_used_day = v_today
  WHERE id = v_daily.id;

  RETURN QUERY SELECT true, v_activated_at + interval '2 hours';
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.activate_turbo_rpc() TO anon, authenticated;
